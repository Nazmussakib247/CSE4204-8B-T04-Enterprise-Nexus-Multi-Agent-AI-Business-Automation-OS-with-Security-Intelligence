...
That introduced a new bug: **the Vercel rewrite proxy silently dropped one of the two `Set-Cookie` headers** on responses that set both cookies at once (login/register), so `refreshToken` never reached the browser. Everyone got logged out once the 15-minute access token expired, and sometimes on a plain reload. **Fixed** by collapsing to a **single `session` cookie** (7-day expiry, sliding — `/auth/refresh` re-signs it if still valid) so there's only ever one `Set-Cookie` header to lose. See `auth.controller.js` (`setAuthCookies`, `generateToken`, `refresh`) and `auth.middleware.js` (`protect` reads `req.cookies.session`). Frontend needed zero changes — cookies are httpOnly, the browser just sends whatever's there.
